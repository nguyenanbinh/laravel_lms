@php
$courses = App\Models\Course::where('status', 1)->orderBy('id')->limit(6)->get();
            ...
            <div class="tab-content" id="myTabContent">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <p class="card-price text-black font-weight-bold">${{ $course->discount_price }}
                                            <span class="before-price font-weight-medium">${{ $course->selling_price
                                                }}</span></p>
                                        @endif
                ...
                <div class="tab-pane fade" id="business{{ $category->id }}" role="tabpanel"
                    ...
                    <div class="row">
                        @php
                        $courses =
                        App\Models\Course::where('category_id',$category->id)->where('status',1)->orderBy('id','DESC')->get();
                        @endphp
    ...
    <div id="tooltip_content_1{{ $item->id }}">
        <div class="card card-item">
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <button type="submit" class="btn theme-btn flex-grow-1 mr-3" onclick="addToCart({{ $item->id }}, '{{ $item->course_name }}','{{ $item->instructor_id }}', '{{ $item->course_name_slug }}' )" ><i class="la la-shopping-cart mr-1 fs-18"></i>
                        Add to Cart
                    </button>
